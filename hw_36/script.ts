...
function getUniqueGenres(array: Film[]) {
   return Array
      .from(new Set(array
         .map(({ genre }) => genre)
         .flat(1)))
}
function getUniqueActors(array: Film[]) {
   return Array
      .from(new Set(array
         .map(({ actors }) => actors)
         .flat(1)))
}
function getSortedMoviesByRating(array: Film[]) {
   return array
      .map((item) => item)
      .sort((a, b) => b.imdbRating - a.imdbRating)
}
function getMovieInformation(array: Film[]) {
   return array.map(({ id, title, released, plot }) => {
      const sortedMovie  = {
         id: id,
         title: title,
         released: released,
